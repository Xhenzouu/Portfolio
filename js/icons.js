/* ==========================================================================
   icons.js
   Lucide (lucide.dev) icon set as SVG strings — MIT licensed.
   Exposes window.portfolioIcons = { name: "<svg…>…</svg>", … }.

   DELIVERY: inline SVG strings, one per icon name. render.js parses each
   string via a detached <template> and imports the resulting <svg> node.
   This is a controlled exception to render.js's "no innerHTML with
   interpolated data" rule: the strings are hardcoded assets, not user
   data, and the template element is never inserted into the live DOM
   as-is — the <svg> child is imported (cloned) into the real target.
